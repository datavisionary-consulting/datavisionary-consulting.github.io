...
Databricks/lakehouse work is what's in demand right now, so the `solutions` array in `content.json` / `content.es.json` leads with it — the Steam review case study is `solutions[0]`, first card on the page, not last. When another Databricks-based case study is added (the global maritime shipping map discussed for a future study is the first candidate), it goes into this same leading position or immediately after it, so the front of the page reads as "this is the Databricks section" at a glance. Keep new Databricks case studies as their own standalone cards (not stacked via `stackGroup` with an unrelated study) unless a future pair genuinely shares the same technique on two different domains, the way the existing stacked pairs do.

## Databricks projects live in their own repository

Unlike every other case study, whose full folder (README, code, figures, results) lives under this repo's `research/`, Databricks projects live in the separate [`datavisionary-consulting/databricks`](https://github.com/datavisionary-consulting/databricks) repository, one subfolder per project — this keeps the real, runnable Databricks notebooks in a repo purpose-built for that, separate from the production website's own repo. This site's `content.json` / `content.es.json` still hold the card copy and narrative for each Databricks case study; only `github_url` and any "Files" links in the write-up point out to the `databricks` repo instead of a local `research/<slug>/` folder. When starting a new Databricks case study, create its folder there first, then write the card content here.
